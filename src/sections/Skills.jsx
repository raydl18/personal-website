          ...
          <Reveal className="lg:col-span-8 space-y-10 md:space-y-12" delay={80}>
            {groups.map((group) => (
              <div key={group.title} className="space-y-4">
                <h4 className="font-dmsans text-[10px] font-bold text-primary uppercase tracking-[0.15em] border-b border-slate-100 pb-2">
                  {group.title}
                </h4>
                <div className="flex flex-wrap gap-2">
                  {group.items.map((item) => (
                    <span
                      key={item}
                      className="px-4 py-2 bg-slate-50 border border-slate-100 text-slate-700 font-dmsans font-bold text-xs rounded hover:bg-white hover:border-slate-200 transition-colors"
                    >
                      {item}
                    </span>
                  ))}
